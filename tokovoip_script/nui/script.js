...
function updateTokovoipInfo(msg) {
	...
	if (msg) {
		document.getElementById('tokovoipInfo').innerHTML = `${msg}`;
	}
	document.getElementById('pluginStatus').innerHTML = `Plugin status: <code>${screenMessage || msg}</code>`;
}

function updateConfig(payload) {
	voip = payload;
	document.getElementById('TSServer').innerHTML = `TeamSpeak server: <code>${voip.plugin_data.TSServer}</code>`;
	document.getElementById('TSChannel').innerHTML = `TeamSpeak channel: <code>${(voip.plugin_data.TSChannelWait) ? voip.plugin_data.TSChannelWait : voip.plugin_data.TSChannel}</code>`;
